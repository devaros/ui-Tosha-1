...
<div style="position: sticky;top:0" >
  ...
  <div class="list_files row px-2">
    ...
    <pre v-if="!edit_mode" contenteditable_ style="white-space:pre-wrap;background-color:#eeeeee;padding: 2px;" v-text="file_content" />

    <textarea v-else id="f_content" multiple name="content" :rows="lines.length+3" _cols="55"
...
function get_api_data(){
  loading.value = true
  fetch(`${backendUrl}show_content/?file_name=${file_nm}`).then(async req => {
    if (req.ok) {
...
function to_view__old2(file_nm, f_type){
